...
namespace JVOS
{
    public static class JWindowGridController
    {
        ...
        public static (IDisposable, IDisposable?) SubscribeToHorizontal(JWindow window, HorizontalAlignment? align)
        {
            var binding = window.Bind(JWindow.WidthProperty, ToHSubject(align));
            IDisposable? binding2 = null;
            if (align == HorizontalAlignment.Right)
                binding2 = window.WindowGridPositionTransform.Bind(TranslateTransform.XProperty, ToHSubject(HorizontalAlignment.Left));
            RefreshHorizontal();
            return (binding, binding2);
        }
        public static (IDisposable, IDisposable?) SubscribeToVertical(JWindow window, VerticalAlignment? align)
        {
            var binding = window.Bind(JWindow.HeightProperty, ToVSubject(align));
            IDisposable? binding2 = null;
            if(align == VerticalAlignment.Bottom)
                binding2 = window.WindowGridPositionTransform.Bind(TranslateTransform.YProperty, ToVSubject(VerticalAlignment.Top));
            RefreshVertical();
            return (binding, binding2);
        }
